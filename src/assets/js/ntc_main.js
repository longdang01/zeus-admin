...
var ntc_main = {
  ...
  init: function () {
    ...
    $("#colorpick").html(
      '<select id="colorop"><option value="">Select a Color:</option>' +
        clrop.join() +
        "</select>"
    );
    ...
    // ntc_main
    //   .setWheel
    //   // window.location.hash.length == 7 ? window.location.hash : "#FFFFFF"
    //   ();
  },
  ...
  setColor: function (clr) {
    ...
    // $("#colorname").html(
    //   n_match[1] +
    //     (n_match[2] ? "<sup id='solid'>solid</sup>" : "<sup>approx.</sup>")
    // );
